// based on https://github.com/cpojer/js-codemod/blob/master/transforms/underscore-to-lodash-native.js
// revised to create import statement for lodash if doesn't exist
...
module.exports = function(fileInfo, { jscodeshift: j }, argOptions) {
  ...
  const underscoreExpressions = ast.find(j.CallExpression, isUnderscoreExpression);
  const lodashIsUsed = underscoreExpressions.__paths.length;
  ...
  if (lodashIsUsed) {
    ...
    const existingImports = ast.find(j.ImportDeclaration);
    if (existingImports.length) {
      j(existingImports.at(0).get()).insertAfter(lodashImport);
    } else {
      ast.get().node.program.body.unshift(lodashImport);
    };
  };
